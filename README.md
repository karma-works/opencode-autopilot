...
1. Add the plugin to that project's `opencode.json`:
...
2. Copy `modes/auto.md` to the target project:
...
3. Start OpenCode and press `Tab` until `auto` is selected.

## Configuration

- `autoMode.maxSteps`: maximum tool calls before autopilot stops
- `autoMode.timeoutMinutes`: wall-clock timeout for a session
- `autoMode.writableRoots`: paths where structured writes can be auto-approved
- `autoMode.allowedNetworkHosts`: hostnames where read-only GET requests can be auto-approved
- `autoMode.judge.provider` / `autoMode.judge.model`: optional secondary judge model selection
